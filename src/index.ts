...
const situationOptions: Situation[] = ["Habitado", "Habitável", "Inabitável", "Inexplorado"]
const situationList = situationOptions.join(`\n`)

function savePlanet(name:string, coordinates: [number, number, number, number], situation: Situation) {
    const planet = {
        name,
        coordinates,
        situation,
...
function updatePlanetSituation(){
    ...
    if (planet){
        while (true) {
            ...
            if (situation && situationOptions.indexOf(situation as Situation) !== -1) {
                const confirmation = confirm(`Deseja atualizar a situação do planeta ${planet.name} para ${situation}`)
                if(confirmation){
                    planet.situation = situation as Situation;
                }
...
function firstMenuOption(){
    ...
    let situationValid: Situation
    while (true) {
        const situation = prompt(`Informe a situação do planeta?\nOpções:\n${situationList}`);

        if (situation && situationOptions.indexOf(situation as Situation) !== -1) {
            situationValid = situation as Situation;
            break; // Sai do loop se a opção for válida
        } else {
            alert(`Opção ${situation} inválida, escolha uma opção válida:\n${situationList}`);
        }
